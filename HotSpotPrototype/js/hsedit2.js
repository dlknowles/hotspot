...
function init(pagenm) {
    pagename = pagenm; //  'CanvasTestbed.aspx';
    setVars();
    setPageElements();
    setStage();
    addImage();
...
function editHotspot(e) {
    ...
    if (hs) {
        ...
        if (hs.shapeType == 'Polygon') editPolygonHotspot(hs);
...
function editPolygonHotspot(hs) {
    // we have to treat polygon shapes differently. 
    // when they are moved, the coordinates remain the same, but the position (x and y) is changed.
    // to get the new coordinates, we must add the new position to each coordinate.
    var len = hs.coordinates.length;

    for (var i = 0; i < len; ++i) {
        // the x coordinates start at 0 and will all be at even indexes.
        // the y coordinates start at 1 and will all be at odd indexes.
        if (isEven(i)) {
            hs.coordinates[i] += selectedShape.getPosition().x;
        } else {
            hs.coordinates[i] += selectedShape.getPosition().y;
        }
    }
}
